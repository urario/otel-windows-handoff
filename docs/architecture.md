...
| 基準 | `urario/zenn-content#68` 対応後の構成 |
| 更新管理 | Git 履歴で管理 |
...
classDiagram
    ...
    Program --> PipelineRunner : creates
    MainWindow --> TelemetrySession : owns for window lifetime
    Program --> TelemetrySession : owns for process command
...
- WinUI でモードを変更すると、関連操作を一時的に無効化し、UI スレッド外で古いセッションを完全に Dispose してから新しいセッションを作る。同じ `ActivitySource` に複数 Provider を重ねない。
- ウィンドウ終了がモード切替と重なった場合は、進行中のバックグラウンド切替を待って得られた最後のセッションを Dispose する。切替完了後に閉じたウィンドウへセッションを再公開しない。
...
Freeze ボタンはパイプライン実行中でもクリック可能である。クリック後は意図どおり UI スレッドが停止するため、30秒間はボタンを含む画面全体が応答しない。これは複数ジョブの Trace が存在する状況で、フリーズ操作固有の root Span を識別するための意図的な設計である。UI の設定変更は実行中に無効化するが、クリック前の診断操作まで無効化しない。
...
測定手順は各条件のウォームアップを `run_index=0` として記録し、集計から除外できるようにする。`off` / `sdk` / `otlp_up` は決定的な全順列を使い、位置と直前条件の偏りを一条件へ集中させない。3条件では6 run、2条件では2 runを1周期とし、完全な均衡には周期の倍数を使う。Collector を停止する `otlp_down` は破壊的な条件なので最後に固定し、時間ドリフトと交絡する制約を CSV と環境記録へ残す。

E3 専用の `collector/otelcol-e3.yaml` は `windowsperfcountersreceiver` を含まない。通常デモのホストメトリクス収集負荷を、アプリの OTel モード比較へ混入させないためである。専用設定だけが構成確認用 health check を `localhost:13134` で公開し、測定スクリプトは OTLP ポートと併せて確認する。OS ファイルキャッシュ、ウイルス対策、他プロセス負荷は統制していないため、結果は絶対性能ではなく同一環境内の条件差として解釈する。
...
| TelemetrySession をアプリ寿命で所有 | UI 操作も計装し、終了順を一元化 | モード変更中は関連操作を無効化し、切替失敗を UI に通知する必要がある |
